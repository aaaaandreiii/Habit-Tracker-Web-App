import { PrismaClient, HabitType, FrequencyType, MealType, Unit } from '@prisma/client';
...
async function main() {
  ...
  await prisma.waterLog.createMany({
    data: [
      { userId: user.id, amount: 250, unit: Unit.ML, dateTime: new Date() },
      { userId: user.id, amount: 500, unit: Unit.ML, dateTime: new Date() },
    ],
  });
